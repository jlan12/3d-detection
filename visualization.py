...
def convert_to_2d(point):
    """
    Used in make_arrow, converts a 3D point to a 2D pixel plane
    """
...
def make_endpoint(point, center, dims, rot_y):
    """
    Used in make_arrow, makes endpoint connected to center of 3D BBox base
    """
...
def make_arrow(calibration, kitti_label, img, rot_y_pred):
    """
    Creates display for ground truth angle and predication angle from a given instance
    
    calibration - str directory to calibration file
    kitti_label - str to kitti ground truth file
    ...
    for line in open(kitti_label):
        ...
        center = convert_to_2d(center)
        endpoint = convert_to_2d(endpoint)
        gt_endpoint = convert_to_2d(gt_endpoint)
